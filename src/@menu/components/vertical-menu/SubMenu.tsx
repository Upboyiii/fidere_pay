...
const SubMenu: ForwardRefRenderFunction<HTMLLIElement, SubMenuProps> = (props, ref) => {
  ...
  const handleSlideToggle = (): void => {
    ...
    // 用户点击，直接切换菜单状态
    // 如果菜单当前是打开的，关闭它；如果当前是关闭的，打开它
    // 传递当前的 active 状态，确保 accordion 模式不会影响其他菜单
  ...
  useEffect(() => {
    const hasActiveChild = confirmUrlInChildren(children, pathname)

    // 只在第一次加载时自动展开包含当前页面的父菜单
    if (!isInitializedRef.current) {
      ...
      // 初始化之后，只更新 active 状态，不做任何自动展开/关闭操作
  ...
  return (
    ...
    >
      ...
      <MenuButton
        ...
        title={title}
        className={classnames(menuClasses.button, { [menuClasses.active]: active }, 'py-3')}
        component={component}
