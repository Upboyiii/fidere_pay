...
const Menu: ForwardRefRenderFunction<HTMLMenuElement, MenuProps> = (props, ref) => {
  ...
  useEffect(() => {
    // 只在第一次加载时处理 openSubmenusRef，初始化展开状态
    if (!isInitializedRef.current) {
      isInitializedRef.current = true
      // 使用 setTimeout 确保所有 SubMenu 的 useEffect 都执行完毕后再更新展开状态
      const timer = setTimeout(() => {
        const newSubmenus = [...openSubmenusRef.current]
        if (newSubmenus.length > 0) {
          setOpenSubmenu(newSubmenus)
        }
